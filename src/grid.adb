...
package body Grid is
   ...
   begin
      ...
      X := 1;
      while X <= GRID_SIZE and not isWinner loop
      ...
      if not isWinner and This.checkSecondDiag then
         isWinner := True;
         This.Winner := This.getCase(1, GRID_SIZE);         
      end if;
   ...
   function checkSecondDiag(This : in T_Grid) return Boolean is
      Ref : T_Case_Value := This.getCase(1,GRID_SIZE);
   begin
      ...
      for X in 2..GRID_SIZE loop
         if Ref /= This.getCase(X,GRID_SIZE+1 - X) then
            return False;
         end if;
      end loop;
